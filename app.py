...
# Configuration
DEV_MODE = os.environ.get('DLMM_DEV_MODE', 'true').lower() == 'true'
...
def main():
    if DEV_MODE:
        ...
        print("Running in development mode (in-process)")
        run_streamlit_app()
    else:
        # Run out-of-process for production
        print("Running in production mode (out-of-process)")
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == '--server':
        # Direct streamlit server invocation
        run_streamlit_app()
    else:
        # Normal execution through main()
        main()